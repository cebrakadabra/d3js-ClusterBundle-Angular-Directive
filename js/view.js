d3app.directive('d3clusterDirective', function() {
    return {
      ...
      link: function postLink(scope, element, attrs) {
		...
		// 		MAKE CHANGES HERE
		// ****************************
		/* Maybe put this in a own directive to be more flexible */
		/* just an idea */
		...
		function setPopupPosition(e){
			...
			if(mouseY < $('#'+elementid+'').offset().top + $('#'+elementid+'').outerHeight()/2){
				//bottom 
			...
			if(mouseX < $('#'+elementid+'').offset().left + $('#'+elementid+'').outerWidth()/2 ){
				//left side
